...
use wasmparser::{
	BlockType, CustomSectionReader, Data, Element, Export, ExternalKind, FuncType, FunctionBody,
	Global, Import, MemoryType, Name, NameSectionReader, Naming, Parser, Payload, Result,
	TableType, Type, TypeRef,
};
...
impl<'a> Module<'a> {
	...
	fn load_name_section(&mut self, reader: &CustomSectionReader<'a>) -> Result<()> {
		let reader = NameSectionReader::new(reader.data(), reader.data_offset())?;

		for name in reader {
			if let Name::Function(map) = name? {
				let mut map = map.get_map()?;

				while let Ok(Naming { index, name }) = map.read() {
					self.name_section.insert(index, name);
				}
			}
		}
	...
	fn load_data(&mut self, data: &'a [u8]) -> Result<()> {
		for payload in Parser::new(0).parse_all(data) {
			match payload? {
				...
				Payload::CustomSection(v) if v.name() == "name" => self.load_name_section(&v)?,
				_ => {}
			}
		}
